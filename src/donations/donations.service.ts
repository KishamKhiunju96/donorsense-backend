...
export class DonationsService {
  ...
  async getStats(orgId: string) {
    ...
    const [allTime, mtd, ytd, count] = await Promise.all([
      this.prisma.donation.aggregate({ where: { orgId }, _sum: { amount: true } }),
      this.prisma.donation.aggregate({ where: { orgId, createdAt: { gte: mtdStart } }, _sum: { amount: true } }),
      this.prisma.donation.aggregate({ where: { orgId, createdAt: { gte: ytdStart } }, _sum: { amount: true } }),
  ...
  async getTrend12m(orgId: string) {
    ...
    for (const d of donations) {
      const key = new Date(d.createdAt).toLocaleString('default', { month: 'short', year: '2-digit' });
      if (key in months) {
    ...
    return Object.entries(months).map(([month, amount]) => ({ month, amount: amount ?? 0 }));
  }
